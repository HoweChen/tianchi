...
require([
  ...
  $.each(apiByGroupAndName, function (index, groupEntries) {
    ...
    $.each(groupEntries, function (titleName, entries) {
      ...
      if (title !== undefined) {
        title.toLowerCase()
          .replace(/[äöüß]/g, function ($0) { return umlauts[$0]; });
  ...
  if (apiProject.header) {
    ...
    if (!found_level1) {    // If no Level 1 tags were found, make a title
      nav.unshift({
        group: "_",
        isHeader: true,
        title: ( apiProject.header.title == null )
          ? locale.__("General")
  ...
  // render pagetitle
  var title = apiProject.title ? apiProject.title : "apiDoc: " +
      ...
      if (groupEntry === entry.group) {
        if (oldName !== entry.name) {
          ...
          api.forEach(function (versionEntry) {
            if (groupEntry === versionEntry.group && entry.name ===
              versionEntry.name) {
  ...
  function _hasTypeInFields(fields) {
    ...
    $.each(fields, function (name) {
      result = result ||
        _.some(fields[name], function (item) { return item.type; });
    });
    return result;
  }
  ...
  $("#versions li.version a").on("click", function (e) {
    ...
    $("article[data-version]").each(function (index) {
      ...
      if (semver.lte(version, selectedVersion)) {
        if ($("article[data-group='" + group + "'][data-name='" + name +
          "']:visible").length === 0) {
          // enable Article
          $("article[data-group='" + group + "'][data-name='" + name +
            "'][data-version='" + version + "']").removeClass("hide");
          // enable Navigation
          $("#sidenav li[data-group='" + group + "'][data-name='" + name +
            "'][data-version='" + version + "']").removeClass("hide");
  ...
  function changeVersionCompareTo(e) {
    ...
    if (compareVersion && articleVersions[group][name][0] === selectedVersion ||
      ...
      fields.article.id = fields.article.group + "-" + fields.article.name +
        "-" + fields.article.version;
      fields.article.id = fields.article.id.replace(/\./g, "_");

      fields.compare.id = fields.compare.group + "-" + fields.compare.name +
      ...
      if (fields._hasTypeInParameterFields !== true && entry.parameter &&
        entry.parameter.fields)
        fields._hasTypeInParameterFields = _hasTypeInFields(
          entry.parameter.fields);

      if (fields._hasTypeInErrorFields !== true && entry.error &&
        entry.error.fields)
        fields._hasTypeInErrorFields = _hasTypeInFields(entry.error.fields);

      if (fields._hasTypeInSuccessFields !== true && entry.success &&
        entry.success.fields)
        fields._hasTypeInSuccessFields = _hasTypeInFields(entry.success.fields);

      if (fields._hasTypeInInfoFields !== true && entry.info &&
        entry.info.fields)
        fields._hasTypeInInfoFields = _hasTypeInFields(entry.info.fields);
      ...
      $content.find(".versions li.version a")
  ...
  function resetArticle(group, name, version) {
    var $root = $("article[data-group='" + group + "'][data-name='" + name +
      "']:visible");
    ...
    $("#sidenav li[data-group='" + group + "'][data-name='" + name +
      "'][data-version='" + version + "']").removeClass("has-modifications");
